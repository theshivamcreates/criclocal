...
import { Users, Shield, Plus, Upload, X, Edit2, Trash2, Info, CheckCircle2, Circle } from "lucide-react";
...
export default function ClubsPage() {
  ...
  const [editingRoles, setEditingRoles] = useState<Record<string, { position: string; isPlaying11: boolean }>>({});
  ...
  useEffect(() => {
    ...
    const unsub = onAuthStateChanged(auth, async (u) => {
      setUser(u);
      if (u) {
        const uDoc = await getDoc(doc(firestore, `users/${u.uid}`));
        if (uDoc.exists()) {
          setUserRole(uDoc.data().primaryRole || "Player");
        }
  ...
  const fetchTeams = async (userId: string) => {
    try {
      ...
      if (myT) {
        ...
        const initialRoles = myT.playerRoles || {};
        for (const p of myT.players) {
          if (!initialRoles[p]) {
            initialRoles[p] = { position: "Utility", isPlaying11: false };
          }
        }
        setMyTeam({ ...myT, playerDetails, playerRoles: initialRoles });
      } else {
        setMyTeam(null);
      }
  ...
  const handleEditClick = () => {
    ...
    setEditingRoles(myTeam.playerRoles || {});
    setTeamCaptainId(myTeam.captainId || null);
    window.scrollTo({ top: 0, behavior: "smooth" });
  };

  const handleRoleChange = (playerId: string, field: "position" | "isPlaying11", value: string | boolean) => {
  ...
  const handleUpdateTeam = async (e: React.FormEvent) => {
    ...
    // Validate playing 11 count
    const playing11Count = Object.values(editingRoles).filter(r => r.isPlaying11).length;
    if (playing11Count > 11) {
      setError("You cannot select more than 11 players for the Playing XI.");
      return;
    }
    ...
    try {
      ...
      if (photoFile) {
        uploadedUrl = await uploadToImageKit(photoFile, `club_${teamName.trim().replace(/\s+/g, '_')}_${Date.now()}.jpg`);
      }
      ...
      await updateTeam(myTeam.id, {
        ...
        bio: teamBio.trim(),
        logoURL: uploadedUrl,
        playerRoles: editingRoles,
        captainId: teamCaptainId || null
      });
  ...
  const handleRemovePlayer = async (playerId: string) => {
    ...
    try {
      ...
      const newRoles = { ...myTeam.playerRoles };
      delete newRoles[playerId];
      
      await updateTeam(myTeam.id, { players: newPlayers, playerRoles: newRoles });
      await fetchTeams(user.uid);
    } catch(err: any) {
  ...
  const handleDeleteTeam = async () => {
    if (!myTeam || !user) return;
    if (!confirm("Are you sure you want to completely delete this club? This action cannot be undone.")) return;
    setIsSubmitting(true);
    try {
  ...
  const openTeamModal = async (team: Team) => {
    ...
    const safeRoles = team.playerRoles || {};
    setSelectedTeam({ ...team, playerDetails, playerRoles: safeRoles });
  };
  ...
  const handleCreateTeam = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let playersArr = [user.uid];
      let coachIdVal: string | null = null;
      let playerRolesVal = {
        [user.uid]: { position: "Forward", isPlaying11: true }
      };
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              ...
              <p className="text-on-surface-variant font-bold text-lg">
                Manage your Football club, set your Playing XI, and discover opponents.
              </p>
            </div>
        ...
        {(isCreating || isEditing) && (
          <div className="mb-16 bg-surface border border-outline rounded-3xl p-8 shadow-sm">
            <div className="flex items-center justify-between mb-8 pb-4 border-b border-outline">
              <h2 className="text-2xl font-black uppercase tracking-widest text-on-surface">{isEditing ? "Edit Club" : "Create New Club"}</h2>
              <button onClick={() => { setIsCreating(false); setIsEditing(false); }} className="text-on-surface-variant hover:text-primary">
                <X size={24} />
              </button>
            </div>

            <form onSubmit={isEditing ? handleUpdateTeam : handleCreateTeam} className="max-w-3xl mx-auto space-y-6">
              {/* Logo Upload */}
              <div className="flex flex-col items-center gap-4 mb-8">
                <div className="relative group cursor-pointer">
                  <div className="w-32 h-32 rounded-full border-[6px] border-surface bg-surface-dim shadow-md overflow-hidden flex items-center justify-center">
                    {photoPreview ? (
                      <img src={photoPreview} alt="Club Logo" className="w-full h-full object-cover" />
                    ) : (
              ...
              <div>
                <label className="block text-xs font-bold text-on-surface-variant uppercase tracking-wider mb-2">Club Name</label>
                <input
                  type="text"
                  required
                  value={teamName}
              ...
              <div>
                <label className="block text-xs font-bold text-on-surface-variant uppercase tracking-wider mb-2">Club Bio (Optional)</label>
                <textarea
                  value={teamBio}
              ...
              {isEditing && myTeam && (
                <div className="space-y-4 pt-6 border-t border-outline mt-6">
                  <div className="flex items-center justify-between mb-4">
                     <h3 className="font-bold text-sm uppercase tracking-widest text-on-surface">Manage Roster</h3>
                     <span className="text-xs font-black uppercase tracking-widest px-3 py-1 bg-primary/10 text-primary rounded-full">
                       {Object.values(editingRoles).filter(r => r.isPlaying11).length} / 11 Playing
                     </span>
                  </div>
                  
                  <div className="grid grid-cols-1 gap-4">
                    {myTeam.playerDetails.map(player => (
                      <div key={player.id} className="flex flex-col sm:flex-row sm:items-center justify-between bg-surface-variant p-4 rounded-xl border border-outline gap-4">
                        <div className="flex items-center gap-3 min-w-[200px]">
                          {player.photoURL ? (
                            <img src={player.photoURL} className="w-10 h-10 rounded-full object-cover shrink-0" />
                          ) : (
                            <div className="w-10 h-10 rounded-full bg-surface-dim flex items-center justify-center font-bold text-primary text-sm shrink-0">
                              {player.name?.[0]?.toUpperCase() || "?"}
                            </div>
                          )}
                          <p className="font-bold text-sm text-on-surface truncate">{player.name}</p>
                        </div>
                        
                        <div className="flex items-center gap-3 flex-1 sm:justify-end">
                          <select
                            value={editingRoles[player.id]?.position || "Utility"}
                            onChange={(e) => handleRoleChange(player.id, "position", e.target.value)}
                            className="bg-surface-dim border border-outline text-xs font-bold text-on-surface px-3 py-2 rounded-lg outline-none focus:border-primary cursor-pointer"
                          >
                          ...
                          <button
                            type="button"
                            onClick={() => handleRoleChange(player.id, "isPlaying11", !editingRoles[player.id]?.isPlaying11)}
                            className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-colors ${
                              editingRoles[player.id]?.isPlaying11 
                                ? 'bg-primary text-on-primary' 
                                : 'bg-surface border border-outline text-on-surface-variant hover:border-primary/50'
                            }`}
                          >
                            {editingRoles[player.id]?.isPlaying11 ? <CheckCircle2 size={14} /> : <Circle size={14} />}
                            {editingRoles[player.id]?.isPlaying11 ? "Playing XI" : "Substitute"}
                          </button>

                          <button
                            type="button"
                            onClick={() => setTeamCaptainId(player.id)}
                            className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-colors ${
                              teamCaptainId === player.id 
                                ? 'bg-emerald-500 text-white' 
                                : 'bg-surface border border-outline text-on-surface-variant hover:border-emerald-500/50 hover:text-emerald-500'
                            }`}
                          >
                            {teamCaptainId === player.id ? "Captain" : "Make Captain"}
                          </button>

                          {player.id !== myTeam.ownerId && (
                            <button type="button" onClick={() => handleRemovePlayer(player.id)} className="text-red-500 hover:text-red-600 p-2 ml-2">
                              <Trash2 size={18} />
                            </button>
                          )}
                        </div>
                  ...
                  <Link href="/players" className="inline-flex items-center gap-2 text-sm text-primary font-bold tracking-widest uppercase hover:underline mt-4 bg-surface p-4 rounded-xl border border-dashed border-outline hover:border-primary/50 w-full justify-center transition-colors">
                    <Plus size={16} /> Recruit More Players
                  </Link>
                </div>
              )}
              ...
              <button
                ...
                className="w-full bg-primary text-on-primary py-4 rounded-xl font-black uppercase tracking-widest hover:bg-primary-container transition-colors disabled:opacity-50 mt-8"
              >
                {isSubmitting ? "Processing..." : isEditing ? "Update Club" : "Create Club"}
              </button>
              {isEditing && (
                <button
                  type="button"
          ...
          <div className="mb-16">
            ...
            <div className="bg-surface border border-primary/30 rounded-3xl p-8 shadow-sm relative overflow-hidden">
              ...
              <div className="flex flex-col md:flex-row gap-8 relative z-10">
                <div className="flex flex-col items-center md:items-start shrink-0 text-center md:text-left border-b md:border-b-0 md:border-r border-outline pb-8 md:pb-0 md:pr-8 md:w-1/3">
                  {myTeam.logoURL ? (
                  ...
                  {myTeam.bio && (
                    <p className="mt-4 text-sm text-on-surface-variant italic leading-relaxed">&quot;{myTeam.bio}&quot;</p>
                  )}
                </div>

                <div className="flex-1">
                  <div className="flex items-center justify-between mb-6">
                    <h4 className="font-black uppercase tracking-widest text-sm text-on-surface">Club Roster</h4>
                    <span className="text-xs font-bold text-on-surface-variant">
                      {myTeam.players.length} Players 
                      <span className="mx-2 text-outline">|</span> 
                      <span className="text-primary">{Object.values(myTeam.playerRoles || {}).filter(r => r.isPlaying11).length} Playing XI</span>
                    </span>
                  </div>
                  
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {/* Sort to show Playing 11 first */}
                    {[...myTeam.playerDetails].sort((a, b) => {
                      const aPlaying = myTeam.playerRoles?.[a.id]?.isPlaying11 ? 1 : 0;
                      const bPlaying = myTeam.playerRoles?.[b.id]?.isPlaying11 ? 1 : 0;
                      ...
                      <div key={p.id} onClick={() => setSelectedPlayerId(p.id)} className="flex items-center gap-4 bg-surface-dim p-4 rounded-xl border border-outline hover:border-primary/50 transition-colors cursor-pointer group">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-on-surface-variant">{myTeam.playerRoles?.[p.id]?.position || "Utility"}</p>
                        </div>
                        <div className="flex flex-col items-end gap-1">
                           {myTeam.playerRoles?.[p.id]?.isPlaying11 ? (
                              <span className="text-[9px] font-black uppercase tracking-widest text-primary bg-primary/10 px-2 py-0.5 rounded">Playing XI</span>
                           ) : (
                              <span className="text-[9px] font-black uppercase tracking-widest text-on-surface-variant bg-surface border border-outline px-2 py-0.5 rounded">Sub</span>
                           )}
                           {p.id === myTeam.ownerId && (
                             <span className="text-[9px] font-black uppercase tracking-widest text-emerald-500 bg-emerald-500/10 px-2 py-0.5 rounded">Captain</span>
                           )}
                        </div>
                      </div>
                    ))}
                    ...
                    {myTeam.ownerId === user?.uid && (
                      <Link href="/players" className="flex flex-col items-center justify-center gap-2 bg-surface-variant/50 border-2 border-dashed border-outline hover:border-primary/50 transition-colors p-4 rounded-xl min-h-[80px] group">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant group-hover:text-primary transition-colors">Recruit Player</span>
                      </Link>
                    )}
                  </div>
                </div>
        ...
        <div>
          ...
          {allTeams.length === 0 ? (
            <div className="text-center py-16 bg-surface-dim rounded-3xl border border-outline">
              ...
              <h3 className="text-xl font-black uppercase tracking-widest text-on-surface mb-2">No Clubs Yet</h3>
              <p className="text-on-surface-variant font-bold">Be the first to create a Football club!</p>
            </div>
          ) : (
                  ...
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-bold text-on-surface-variant">
                        ...
                        {Object.values(team.playerRoles || {}).filter(r => r.isPlaying11).length} Playing XI
                      </p>
                    </div>
                  </div>
                  {team.bio && (
      ...
      {selectedTeam && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm animate-fade-in">
          <div className="bg-surface border border-outline rounded-3xl w-full max-w-md overflow-hidden shadow-2xl relative max-h-[90vh] flex flex-col">
            ...
            <div className="relative h-48 bg-surface-dim flex items-center justify-center p-6 border-b border-outline shrink-0">
            ...
            <div className="p-8 relative z-10 bg-surface flex-1 overflow-y-auto">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between border-b border-outline pb-2">
                  <h3 className="font-black text-sm uppercase tracking-widest text-on-surface-variant">Roster</h3>
                  <span className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant">{selectedTeam.players.length} Players</span>
                </div>
                <div className="grid gap-3">
                  {[...selectedTeam.playerDetails].sort((a, b) => {
                    const aPlaying = selectedTeam.playerRoles?.[a.id]?.isPlaying11 ? 1 : 0;
                    const bPlaying = selectedTeam.playerRoles?.[b.id]?.isPlaying11 ? 1 : 0;
                    return bPlaying - aPlaying;
                  }).map((player: any) => (
                    <div 
                      key={player.id} 
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-bold text-sm text-on-surface group-hover:text-primary transition-colors truncate">{player.name}</p>
                        <p className="text-[10px] text-on-surface-variant uppercase tracking-widest mt-0.5">{selectedTeam.playerRoles?.[player.id]?.position || "Utility"}</p>
                      </div>
                      <div className="flex flex-col items-end gap-1 shrink-0">
                        {selectedTeam.playerRoles?.[player.id]?.isPlaying11 ? (
                           <span className="text-[8px] font-black uppercase tracking-widest text-primary bg-primary/10 px-1.5 py-0.5 rounded">Playing XI</span>
                        ) : (
                           <span className="text-[8px] font-black uppercase tracking-widest text-on-surface-variant bg-surface border border-outline px-1.5 py-0.5 rounded">Sub</span>
                        )}
                        {player.id === selectedTeam.ownerId && (
                          <span className="text-[8px] font-black uppercase tracking-widest text-emerald-500 bg-emerald-500/10 px-1.5 py-0.5 rounded">Captain</span>
                        )}
                      </div>
                    </div>
                  ))}
