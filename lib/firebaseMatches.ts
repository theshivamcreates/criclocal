...
export function subscribeToMatch(
  ...
  return onValue(ref(database, `matches/${matchId}`), (snapshot) =>
    ...
    (match: Match | null) => {
      ...
      if (updates.nextBatsmanName) {
        if (innings.batsmen[strikerKey]?.status === "out") {
          ...
          innings.batsmen[newKey] = {
            name: updates.nextBatsmanName,
            runs: 0,
            balls: 0,
            fours: 0,
            sixes: 0,
  ...
  return runTransaction(
    ...
    (match: Match | null) => {
      ...
      if (match.currentInnings === 1) {
        ...
        const battingTeam =
          match.innings["1"].battingTeam === "team1" ? "team2" : "team1";
