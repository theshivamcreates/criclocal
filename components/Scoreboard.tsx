...
export function Scoreboard({
  ...
  const target =
    match.currentInnings === 2 && inningsOne ? inningsOne.runs + 1 : null;
    ...
    >
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="text-right">
          {innings.isFreeHit && (
            <div className="mb-1">
              <span className="inline-block animate-pulse rounded-md bg-primary px-2 py-0.5 text-xs font-black tracking-wide text-white">
                FREE HIT
              </span>
            </div>
          )}
          <p className="text-lg font-black">
            ({getOversDisplay(innings.balls)}/{match.meta.overs})
          </p>
          <p className="text-sm text-slate-500">
            RR {getRunRate(innings.runs, innings.balls)}
          </p>
          {target ? (
            <p className="text-sm text-slate-500">
              Need {target - innings.runs} at{" "}
              {getRequiredRunRate(target, innings.runs, ballsLeft)}
            </p>
          ) : null}
        </div>
      </div>
      ...
      <div
        className={
          compact ? "space-y-1" : "mt-5 rounded-lg border border-slate-100 px-3"
        }
      >
        {!compact ? (
          <div className="grid grid-cols-[1fr_2rem_2rem_2rem_2rem] sm:grid-cols-[1fr_3rem_3rem_3rem_3rem] gap-1 sm:gap-2 border-b border-slate-100 py-2 text-[10px] sm:text-xs font-black uppercase text-slate-500">
            <span>Batter</span>
