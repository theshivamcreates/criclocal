...
export default function FootballDashboardPage() {
  ...
  async function handleCreateTournament(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const { requireDb } = await import("@/lib/firebaseFootball");
      await set(dbRef(requireDb(), `football/tournaments/${tId}`), {
        name: tournamentName,
        entryFee: tEntryFee,
        ...
        createdAt: Date.now(),
        teams: {},
        settings: { maxPlayersPerTeam: 25, matchDurationMinutes: 90 },
      });
      setTournamentName("");
      setTEntryFee("");
      setTStartDate("");
    ...
    try {
      ...
      const match: FootballMatch = {
        meta: {
          team1: form.team1,
          team2: form.team2,
          ...(team1LogoUrl ? { team1Logo: team1LogoUrl } : {}),
          ...(team2LogoUrl ? { team2Logo: team2LogoUrl } : {}),
      ...
      setMessage(
        `Match created. Open /football/match/${matchId}/score to start scoring.`,
      );
    } catch (err: any) {
      setMessage(
        ...
        <aside className="rounded-lg border border-outline bg-surface p-4 shadow-sm h-fit">
          ...
          {activeTab === "matches" ? (
            <form className="space-y-4" onSubmit={handleCreate}>
                  ...
                  {form.team1Roster.map((player, idx) => (
                    <div key={idx} className="flex gap-2">
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          const newRoster = [...form.team1Roster];
                          newRoster[idx].role = e.target.value as FootballPlayer["role"];
                          setForm({ ...form, team1Roster: newRoster });
                        }}
                      >
              ...
              >
                <Plus size={18} />
                {isUploading ? "Creating..." : "Create Football Match"}
              </button>
            </form>
          ) : (
            <form className="space-y-4" onSubmit={handleCreateTournament}>
              ...
              >
                <Plus size={18} />
                {isUploading ? "Creating..." : "Create Football Tournament"}
              </button>
            </form>
          )}
          {message ? (
          ...
          {activeTab === "matches" ? (
            <>
              <div className="mb-4 flex items-center justify-between">
                <h2 className="text-xl font-black">Football Matches</h2>
              ...
              <div className="space-y-3">
                {visibleMatches.length ? (
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          {match.meta.status !== "completed" && (
                            <Link
                              ...
                              href={`/football/match/${id}/overlay`}
                            >
                          ...
                          <button
                            onClick={() => {
                              if (
                                ...
                                import("@/lib/firebaseFootball").then(
                                  ({ deleteFootballMatch }) =>
                                    deleteFootballMatch(id)
                  ...
                  <div className="rounded-lg border border-dashed border-outline bg-surface p-8 text-center text-on-surface-variant">
                    Created football matches will appear here.
                  </div>
                )}
              </div>
            </>
            ...
            <>
              <div className="mb-4 flex items-center justify-between">
                <h2 className="text-xl font-black">Football Tournaments</h2>
                <span className="text-sm font-bold text-on-surface-variant">
              ...
              <div className="space-y-3">
                {visibleTournaments.length ? (
                  ...
                  <div className="rounded-lg border border-dashed border-outline bg-surface p-8 text-center text-on-surface-variant">
                    Your football tournaments will appear here.
                  </div>
                )}
              </div>
            </>
